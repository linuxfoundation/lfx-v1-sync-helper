...
// atomicInt64Wrapper is a thin wrapper around atomic.Int64 that keeps
// the two dualMappingStore instrumentation counters (queueDepth,
// droppedEnqueues) lock-free. Every enqueueMirror bumps queueDepth
// and every drop bumps droppedEnqueues; on the write hot path this
// removes an unnecessary sync.Mutex acquire that would otherwise show
// up in profiles under sustained bursts. Tests observe state via
// load() which is a plain atomic.Load.
type atomicInt64Wrapper struct {
	v atomic.Int64
}

func (a *atomicInt64Wrapper) add(delta int64) int64 { return a.v.Add(delta) }
func (a *atomicInt64Wrapper) load() int64           { return a.v.Load() }
