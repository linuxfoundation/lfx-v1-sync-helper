...
func backfillV1MappingsToPostgres(ctx context.Context, pool *pgxpool.Pool, dryRun bool) (res backfillV1MappingsToPostgresResult, err error) {
	...
	if !dryRun {
		...
		// serialising the online backfill. The literal spells
		// "V1MAPBKF" in ASCII (V1 MAPpings BacKFill), chosen to be
		// unique to this backfill and to be recognisable at a glance
		// in pg_locks output, matching the mnemonic convention used
		// by internal/schema/schema.go's schema-bootstrap lock
		// ("V1SYNCHL"). Using an unstructured magic number here would
		// invite silent collisions with lock keys chosen by other
		// services sharing the same Postgres cluster.
		const v1MappingsBackfillLockKey int64 = 0x5631_4D41_5042_4B46 // "V1MAPBKF"
