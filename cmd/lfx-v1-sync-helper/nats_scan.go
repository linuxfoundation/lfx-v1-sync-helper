...
// cfg.NATSFetchMaxWait for backfill scans). A zero or negative value falls
// back to defaultNATSFetchMaxWait.
...
func ScanSubjectDataStreamRange(ctx context.Context, js jetstream.JetStream, streamName, subjectFilter string, startSeq, endSeq uint64, opTimeout time.Duration, cb SubjectDataCallback) (int, int, error) {
	...
	// The scan loop is factored into scanSubjectDataStreamRangeWith so
	// tests can inject a getMsgFn without needing a real JetStream
	// connection. See nats_scan_test.go.
	getFn := func(ctx context.Context, seq uint64, filter string) (*jetstream.RawStreamMsg, error) {
		return stream.GetMsg(ctx, seq, jetstream.WithGetMsgSubject(filter))
	}
	return scanSubjectDataStreamRangeWith(ctx, getFn, streamName, subjectFilter, startSeq, endSeq, opTimeout, cb)
}

// getMsgFn abstracts a single next-by-subj GetMsg call so the scan loop
// can be exercised without a live JetStream. Signature matches the
// call the production adapter makes: ctx, requested seq, subject filter.
type getMsgFn func(ctx context.Context, seq uint64, subjectFilter string) (*jetstream.RawStreamMsg, error)

// scanSubjectDataStreamRangeWith is the testable core of
// ScanSubjectDataStreamRange. It uses the injected getMsgFn instead of
// resolving a real jetstream.Stream, so unit tests can construct a
// hand-crafted sequence timeline and assert the loop's boundary
// semantics (adjacent-range partitioning, half-open [start, end),
// DEL/PURGE classification, callback-error cancellation, context
// cancellation).
func scanSubjectDataStreamRangeWith(ctx context.Context, get getMsgFn, streamName, subjectFilter string, startSeq, endSeq uint64, opTimeout time.Duration, cb SubjectDataCallback) (int, int, error) {
	if startSeq == 0 {
		startSeq = 1
	}
	if opTimeout <= 0 {
	...
	if cb == nil {
		return 0, 0, fmt.Errorf("scanSubjectDataStreamRangeWith: nil callback")
	}
	var (
		visits     int
		tombstoned int
	...
	for {
		...
		callCtx, cancelCall := context.WithTimeout(ctx, opTimeout)
		msg, getErr := get(callCtx, seq, subjectFilter)
		cancelCall()
