...
- **Emit**: fresh `v1KV.Get` → re-apply pre-filters → re-check parent-project-mapping liveness → revision-checked `v1KV.Update`; a revision mismatch (`isRevisionMismatchError`) counts as `skipped_revision_race`, not an error, and is still settle-polled (rather than left permanently unresolved) since the concurrent writer that won may itself be creating the mapping.
...
- **Flags**: `--emit-rate` (default 2.0/s, shared flag with `--backfill-projects`), `--force` (shared flag). Reuses `--dry-run` and `--limit`, defaulting `--limit` to unlimited (0) unless explicitly passed, matching `--backfill-projects`'s rationale that the acceptance criterion is every unmapped v1 committee created in one run.
- **Dry-run**: validates candidate selection only; it cannot observe the downstream create. Reports the full candidate/skip counters.
- **Summary log fields**: `scanned`, `mappings_live`, `mappings_tombstoned`, `low_mappings_count`, `candidates`, `emitted`, `remaining_unmapped`, `errors`, plus the `skipped_*` breakdown (`skipped_already_mapped`, `skipped_tombstoned`, `skipped_soft_deleted`, `skipped_empty_value`, `skipped_undecodable`, `skipped_no_sfid`, `skipped_missing_required`, `skipped_v2_authored`, `skipped_parent_unmapped`, `skipped_revision_race`, `skipped_limit`, `skipped_missing`, `skipped_settle_timeout`) (shared via `backfillCommitteesResult.logFields()`). `remaining_unmapped` is recomputed from final settle state at the end of a live run.
- **Manifest**: `manifests/backfill-committees-job.yaml`, shipped dry-run by default.
