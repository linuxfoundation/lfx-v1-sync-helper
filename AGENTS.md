...
- **Emit**: fresh `v1KV.Get` → re-apply pre-filters → revision-checked `v1KV.Update`; a revision mismatch (`isRevisionMismatchError`) counts as `skipped_revision_race`, not an error — the live path already handled it. Every level's emitted SFIDs are settle-polled even if the per-run `--limit` was hit partway through that level (otherwise those successful creates would be double-counted as unmapped); an emission that never settles (downstream create failure or a genuine settle-poll timeout) increments `skipped_settle_timeout` and causes the run to return a non-nil error, distinct from the expected/non-failure `skipped_limit` and `skipped_revision_race` outcomes.
