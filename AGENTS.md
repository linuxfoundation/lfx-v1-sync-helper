...
- **Emit**: fresh `v1KV.Get` → re-apply pre-filters → re-check parent-project-mapping liveness → re-check `--check-committee-names` (if enabled) → revision-checked `v1KV.Update`; a revision mismatch (`isRevisionMismatchError`) counts as `skipped_revision_race`, not an error, and is still settle-polled (rather than left permanently unresolved) since the concurrent writer that won may itself be creating the mapping.
